...
def cross_entropy(pred,
    ...
    if (avg_factor is None) and reduction == 'mean':
        if class_weight is None:
            ...
            #label_weights = class_weight[label[label != ignore_index]].to(device=class_weight.device)
            label_weights = torch.zeros_like(label, dtype=class_weight.dtype,device='cuda:0')
